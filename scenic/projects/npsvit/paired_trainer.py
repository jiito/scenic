...
from scenic.projects.npsvit import metrics as sv_metrics
...
def train_step(
  ...
  logging.info("Training state params %s", train_state.params)
  logging.info("Training state model state %s", train_state.model_state)
  ...
  def training_loss_fn(params):
    ...
    logits, new_model_state = flax_model.apply(
        variables,
        # batch['inputs'], # IMAGES [support, query]
        batch['support'],
        batch['query'], 
        mutable=['batch_stats'],
        train=True,
    ...
    logging.info('new_model_state: %s', new_model_state)
...
def eval_step(
  ...
  logits = flax_model.apply(
      variables, batch['support'], batch['query'], train=False, mutable=False, debug=debug)
  metrics = metrics_fn(logits, batch)
  if all_gather:
  ...
  (params, model_state, num_trainable_params,
   gflops) = train_utils.initialize_model(
       ...
       input_spec=[(dataset.meta_data['input_shape'],
                    dataset.meta_data.get('input_dtype', jnp.float32))] , #duplicate tuple
       config=config,
       rngs=init_rng)

  logging.info('Number of trainable parameters: %d', num_trainable_params)
  logging.info('GFLOPs: %.2f', gflops)
  logging.info('Model initialized.')
  logging.info("params:", jax.tree_map(np.shape, params)) 
  # MODEL STATE IS {} HERE
  logging.info("model state:", model_state) 
