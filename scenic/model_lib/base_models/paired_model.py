...
def paired_metrics_function(
    logits: jnp.ndarray,
    batch: base_model.Batch,
    target_is_onehot: bool = False,
    metrics: base_model.MetricNormalizerFnDict = _PAIRED_METRICS,
...
def normalized_contrastive_loss(
    logits: jnp.ndarray,
  ...
  if weights is not None:
    ...
    normalization = jnp.prod(one_hot_targets.shape[:-1])

  """
  Loss = Y⋅D2 + (1-Y)⋅max(0,m-D)2
  Y is 1 for matching genotypes, 0 otherwise
  D is distance between pairs of embeddings
  m is the margin constant, typically 1

  """
  unnormalized_contrastive_loss = jnp.square(one_hot_targets) * jnp.square(logits) + (1 - one_hot_targets) * jnp.square(jnp.maximum(0, 1 - logits))
  return jnp.sum(unnormalized_contrastive_loss) / (normalization + 1e-8)


class PairedModel(base_model.BaseModel):
  """Defines commonalities between all paired models.
  ...
  def loss_function(self,
    ...
    Returns:
      Total loss.
    """
    # TODO: MODIFY to use contrastive loss
    ...
    ct_loss = normalized_contrastive_loss(logits, one_hot_targets, weights)
    if self.config.get('l2_decay_factor') is None:
      total_loss = ct_loss
    else:
      l2_loss = model_utils.l2_regularization(model_params)
      total_loss = ct_loss + 0.5 * self.config.l2_decay_factor * l2_loss
